...
*** ruby debugger (dap-mode)
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (defun rbenv-rspec-path ()
        (setq w/ruby-gemset-path (replace-regexp-in-string  "\n\+$" "" (shell-command-to-string "gem env home 2>/dev/null")))
        (expand-file-name
         (format "%s/bin/rspec" w/ruby-gemset-path)))
      ...
      (general-define-key
       ...
       "tdt" '(dap-ruby-run-test-at-point :which-key "Run test at point")
       "tdb" '(dap-ruby-run-test :which-key "Run all tests")
