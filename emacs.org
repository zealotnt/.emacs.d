   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package general
       ...
       (general-define-key
        ...
        "s-t s-i" 'indent-tools-hydra/body
        ...
        "s-; s-b" 'z/visual-by-boundary

        ;; combination with super+k
        ;"s-; s-t" '(:ignore t :which-key "text stuffs")
        "s-; s-t" 'figlet-preview-fonts
        "s-; t" 'figlet-preview-fonts

        "s-; s-u" 'z/epoch-unix-to-time
        "s-; u" 'z/epoch-unix-to-time
        "s-; p" 'z/query-prometheus
        "s-; s-p" 'z/query-prometheus

        "s-; s-g" '(:ignore t :which-key "git stuffs")
        "s-; s-g c" 'hydra-smerge/body
        "s-; s-g s-c" 'hydra-smerge/body
        "s-; s-g g" 'magit-status ;status here
        "s-; s-g s-g" 'magit-status ;status here
        "s-; s-g n" 'z/magit-status ;status in new frame
        "s-; s-g s-n" 'z/magit-status ;status in new frame
        "s-; s-g b" 'magit-blame-echo
        "s-; s-g s-b"   'magit-blame-echo
        "s-; s-g q"   'magit-blame-quit
        "s-; s-g t" 'hydra-git-timemachine-on
        "s-; s-g s-t" 'hydra-git-timemachine-on
        "s-; s-g l" 'magit-log-buffer-file
        "s-; s-g s-l" 'magit-log-buffer-file

        ;"s-; s-g s-q"   'magit-blame-quit

        "s-; s-s"   'z/neotree-project-dir-toggle

        "s-; s-d" '(:ignore t :which-key "decoding stuffs")
        "s-; s-d u" 'z/epoch-unix-to-time
        "s-; s-d s-u" 'z/epoch-unix-to-time
        "s-; s-d d" 'base64-decode-region
        "s-; s-d s-d" 'base64-decode-region
        "s-; s-d 6" 'base64-decode-region
        "s-; s-d s-6" 'base64-decode-region

        "s-; s-f" '(:ignore t :which-key "files stuffs")
        "s-; s-f b" 'magit-open-repo
        "s-; s-f s-b" 'magit-open-repo
        "s-; s-f n" 's/show-copy-buffer-file-name
        "s-; s-f s-n" 's/show-copy-buffer-file-name
        "s-; s-f r"   'w/show-copy-relative-file-path
        "s-; s-f s-r" 'w/show-copy-relative-file-path
        "s-; s-f a"    'z/show-copy-absolute-path
        "s-; s-f s-a"  'z/show-copy-absolute-path
        "s-; s-f l"   'w/show-copy-relative-file-path-with-line
        "s-; s-f s-l" 'w/show-copy-relative-file-path-with-line
        "s-; s-f O" 'z/open-file-in-remote-git
        "s-; s-f o" 'xah-open-in-chrome
        "s-; s-f s-o" 'xah-open-in-chrome
        "s-; s-f d" 'z/show-copy-containing-folder-path
        "s-; s-f s-d" 'z/show-copy-containing-folder-path
        "s-; s-f D" 'xah-show-in-desktop
        "s-; s-f s-D" 'xah-show-in-desktop
        "s-; s-f f" 'z/which-function
        "s-; s-f s-f" 'z/which-function
        "s-; s-f p" 'w/show-buffer-project-path
        "s-; s-f s-p" 'w/show-buffer-project-path
        "s-; s-[" 'aj-toggle-fold

        "s-; s-e" '(:ignore t :which-key "eval-elisp")
        "s-; s-e s" 'shell-command
        "s-; s-e s-s" 'shell-command
        "s-; s-e h" 'z/eval-last-sexp
        "s-; s-e s-h" 'z/eval-last-sexp
        "s-; s-e e" 'z/eval-region
        "s-; s-e s-e" 'z/eval-region
        "s-; s-e p" 'z/eval-paragraph
        "s-; s-e s-p" 'z/eval-paragraph

        "s-; s-w" 'toggle-truncate-lines
        "s-; s-i" 's/show-copy-buffer-file-name

        "s-; s-r" '(:ignore t :which-key "restart stuffs")
        "s-; s-r c" 'z/sync-config
        "s-; s-r r" 'revert-buffer

        "s-; s-j" 'git-gutter:next-hunk
        "s-; s-k" 'git-gutter:previous-hunk

        "s-; c" 'atomic-chrome-close-current-buffer
        "s-; s-c" 'atomic-chrome-close-current-buffer
...
** xdg config
   #+BEGIN_SRC emacs-lisp
     ...
     (global-set-key (kbd "s-; s-e s-x") 'xah-run-current-file)
     (global-set-key (kbd "s-; s-e x") 'xah-run-current-file)
